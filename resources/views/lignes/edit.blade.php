...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Éditer la ligne {{ $ligne->nom }}</div>

                <div class="card-body">
                    <form method="POST" action="{{ route('lignes.update', $ligne->id) }}">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('ville_depart') is-invalid @enderror" id="ville_depart" name="ville_depart" required>
                                        ...
                                        @foreach($villes as $ville)
                                            <option value="{{ $ville->nom }}" {{ old('ville_depart', $ligne->ville_depart) == $ville->nom ? 'selected' : '' }}>{{ $ville->nom }}</option>
                                        @endforeach
                                    </select>
                                    @error('ville_depart')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('ville_arrivee') is-invalid @enderror" id="ville_arrivee" name="ville_arrivee" required>
                                        <option value="">-- Sélectionner --</option>
                                        @foreach($villes as $ville)
                                            <option value="{{ $ville->nom }}" {{ old('ville_arrivee', $ligne->ville_arrivee) == $ville->nom ? 'selected' : '' }}>{{ $ville->nom }}</option>
                                        @endforeach
                                    </select>
                                    @error('ville_arrivee')
                                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nom') is-invalid @enderror" id="nom" name="nom" value="{{ old('nom', $ligne->nom) }}" readonly>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                        <option value="Aller" {{ old('type', $ligne->type) == 'Aller' ? 'selected' : '' }}>Aller (départ de Parakou)</option>
                                        <option value="Retour" {{ old('type', $ligne->type) == 'Retour' ? 'selected' : '' }}>Retour (arrivée à Parakou)</option>
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="time" class="form-control @error('horaire') is-invalid @enderror" id="horaire" name="horaire" value="{{ old('horaire', substr($ligne->horaire, 0, 5)) }}" required>
                                    @error('horaire')
                                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                    @enderror
                                </div>
                        ...
                        <div class="mb-3" id="ligne_retour_container">
                            ...
                            <select class="form-select @error('ligne_retour_id') is-invalid @enderror" id="ligne_retour_id" name="ligne_retour_id">
                                ...
                                @foreach($lignesRetour as $ligneRetour)
                                    <option value="{{ $ligneRetour->id }}" {{ old('ligne_retour_id', $ligne->ligne_retour_id) == $ligneRetour->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="est_ligne_nord" name="est_ligne_nord" value="1" {{ old('est_ligne_nord', $ligne->est_ligne_nord) ? 'checked' : '' }}>
                            <label class="form-check-label" for="est_ligne_nord">Est-ce une ligne Nord ?</label>
                        </div>

                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="aller_retour_meme_jour" name="aller_retour_meme_jour" value="1" {{ old('aller_retour_meme_jour', $ligne->aller_retour_meme_jour) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('distance_km') is-invalid @enderror" id="distance_km" name="distance_km" value="{{ old('distance_km', $ligne->distance_km) }}">
                            @error('distance_km')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('duree_estimee') is-invalid @enderror" id="duree_estimee" name="duree_estimee" value="{{ old('duree_estimee', $ligne->duree_estimee) }}">
                            @error('duree_estimee')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
